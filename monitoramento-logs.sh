...
LOG_DIR="../projetos-devops/myapp/logs"
...
find $LOG_DIR -name "*.log" -print0 | while IFS= read -r -d '' arquivo; do
	...
	num_words=$(wc -w < "${arquivo}.unico") 
	num_lines=$(wc -l < "${arquivo}.unico")

	nome_arquivo=$(basename "${arquivo}.unico")

	echo "Resultado: $nome_arquivo" >> "${ARQUIVO_DIR}/log_stats_$(date +%F).txt"
	echo "Número de linhas: $num_lines" >> "${ARQUIVO_DIR}/log_stats_$(date +%F).txt"
	echo "Número de palavras: $num_words" >> "${ARQUIVO_DIR}/log_stats_$(date +%F).txt"
